...
IP=$( ip -4 a | grep inet | grep -v '127.0.0.1\|:\|bridge' | cut -d " " -f6 | tr "\n\r" " " )
IP_chk=$( curl -s ifconfig.io )
if [ ! -z $IP_chk ]
then
	IP_ext="$IP_chk <>";
else
	IP_ext="No internet x>";
fi

if [ "$color_prompt" = yes ]; then
    PROMPT=$'%F{%(#.green.green)}┌── [ $(date +"%d-%b-%Y") %* ] - [ $IP_ext $IP] - ${debian_chroot:+($debian_chroot)──}$(venv_info)[ %B%F{%(#.red.red)}%n%(#.💀.@)%m%b%F{%(#.green.green)} ]\n├── [ %B%F{reset}%(6~.%-1~/…/%4~.%5~)%b%F{%(#.green.green)} ]\n└── %B%(#.%F{red}#.%F{red}$)%b%F{reset} '
